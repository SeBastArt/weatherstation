...
namespace e_ink_display_project
{
    ...
    public:
        ...
        display();
        ~display();
        static auto write_room(const char* name, const double temperature, uint16_t x_start, uint16_t y_start) -> void;
        static auto write_temperature(double temperature, UWORD x_start, UWORD y_start) -> void;
        auto write_date(UWORD x_start, UWORD y_start) const -> void;
        auto write_time(UWORD x_start, UWORD y_start) const -> void;
        auto draw_lines() const -> void;
